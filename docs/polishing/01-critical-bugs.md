...
- [ ] **Chat history loses `message_id`** — `frontend/src/hooks/useRoom.ts:170`
...
- [ ] **Video reaction `setTimeout` leak** — `frontend/src/hooks/useRoom.ts:351-353`
...
- [ ] **Host race condition on simultaneous join** — `app/handlers/socket_events.py:183-199`
...
- [ ] **Queue title not validated** — `app/handlers/socket_events.py` (`handle_queue_add`)
  - Empty / whitespace-only titles are accepted into the queue.
  - Fix: reject after `.strip()`; emit error to sender.

- [ ] **Queue reorder bounds not validated in handler** — `app/handlers/socket_events.py` (`handle_queue_reorder`)
...
- [ ] **User name length unbounded** — `app/handlers/socket_events.py` (`handle_create_room`, `handle_join_room`)
...
- [ ] **Video reaction silent failure** — `app/handlers/socket_events.py` (`handle_video_reaction`)
