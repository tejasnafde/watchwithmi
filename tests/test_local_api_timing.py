...
def analyze_results(results: Dict[str, List[float]]) -> None:
    ...
    for query, time in sorted(fast_queries, key=lambda x: x[1]):
        print(f"   {time:.2f}s - '{query}'")
    
    print(f"\n🚶 Medium Queries (3-8s):")
    for query, time in sorted(medium_queries, key=lambda x: x[1]):
        print(f"   {time:.2f}s - '{query}'")
    
    print(f"\n🐌 Slow Queries (> 8s):")
    for query, time in sorted(slow_queries, key=lambda x: x[1]):
        print(f"   {time:.2f}s - '{query}'")
...
def test_watchwithmi_integration():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f" WatchWithMi search successful!")
